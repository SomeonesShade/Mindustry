...
abstract class UnitComp implements Healthc, Physicsc, Hitboxc, Statusc, Teamc, Itemsc, Rotc, Unitc, Weaponsc, Drawc, Boundedc, Syncc, Shieldc, Displayable, Ranged, Minerc, Builderc, Senseable, Settable{
    ...
    public void destroy(){
        ...
        float explosiveness = 2f + item().explosiveness * stack().amount * type.itemPropertyMultiplier * 1.53f;
        float flammability = item().flammability * stack().amount * type.itemPropertyMultiplier / 1.9f;
        float power = item().charge * Mathf.pow(stack().amount * type.itemPropertyMultiplier, 1.11f) * 160f;
